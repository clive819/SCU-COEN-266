...
def iterativeDeepeningSearch(problem):
    ...
    lim = 0
    while True:
        if dfs(problem.getStartState(), lim):
            return path
        lim += 1
        visited.clear()
...
class PriorityItem(object):
    def __init__(self, state, g, h, actions):
        self.state = state
        self.g = g
        self.h = h
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    visited = set()
    queue = PriorityQueue()
    queue.put(PriorityItem(problem.getStartState(), 0, 0, []))

    while queue:
        item = queue.get()
        visited.add(item.state)
        if problem.goalTest(item.state):
            return item.actions

        for action in problem.getActions(item.state):
            nextState = problem.getResult(item.state, action)
            if nextState not in visited:
                h = heuristic(nextState, problem)
                g = problem.getCost(item.state, action)

                newItem = PriorityItem(nextState, item.g + g, h, item.actions + [action])
                queue.put(newItem)
